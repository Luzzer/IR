...
class TF_IDF:
	...
	def lm_mixture_model(self, term_freq, doc_now_len, now_all_term_freq):
		...
		return math.log(1.0 + (lm_lambda * (term_freq / doc_now_len)) / ((1.0 - lm_lambda) * (now_all_term_freq / all_term_count)))
		...
		for term in query:
			...
			for doc in docs:
				...
				#	score_lst[doc_id] += self.word_tf(term_freq, doc_avg_len, doc_now_len)*self.word_idf(idf_N, term_doc_freq)
					score_lst[doc_id] += self.lm_mixture_model(float(term_freq), float(doc_now_len), float(now_all_term_freq))
				else:
				#	score_lst[doc_id] = self.word_tf(term_freq, doc_avg_len, doc_now_len)*self.word_idf(idf_N, term_doc_freq)
					score_lst[doc_id] = self.lm_mixture_model(float(term_freq), float(doc_now_len), float(now_all_term_freq))
		self.conn.close();
...
if __name__ == "__main__":
	...
	scorer.save_sorted_tfidf("AP88_result2.txt", "topics.202-250.txt")
	#print "="*50
	#while True:
